...
impl SteamCmdManager {
    ...
    /// Get the steamcmd directory path
    ...
    /// Check if a workshop item is downloaded
    pub fn is_workshop_item_downloaded(&self, game_app_id: u32, workshop_id: u64) -> bool {
        self.get_workshop_content_dir(game_app_id)
            .join(workshop_id.to_string())
            .exists()
    }

    /// Run a custom SteamCMD command (for advanced usage)
    pub fn run_custom_command(&self, args: &[String]) -> Result<()> {
        self.run_steamcmd_with_args(args)
    }
